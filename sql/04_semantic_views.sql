...
AS
TABLES (
    GUESTS AS IDENTIFIER($FULL_PREFIX || '.GOLD.GUEST_360_VIEW_ENHANCED') PRIMARY KEY (GUEST_ID)
)
DIMENSIONS (
...
AS
TABLES (
    SCORES AS IDENTIFIER($FULL_PREFIX || '.GOLD.PERSONALIZATION_SCORES_ENHANCED') PRIMARY KEY (GUEST_ID),
    GUESTS AS IDENTIFIER($FULL_PREFIX || '.GOLD.GUEST_360_VIEW_ENHANCED') PRIMARY KEY (GUEST_ID)
)
RELATIONSHIPS (
...
AS
TABLES (
    AMENITIES AS IDENTIFIER($FULL_PREFIX || '.GOLD.AMENITY_ANALYTICS') PRIMARY KEY (METRIC_MONTH, AMENITY_CATEGORY, LOCATION)
)
DIMENSIONS (
