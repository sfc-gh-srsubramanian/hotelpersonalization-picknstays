...
TABLES (
    AMENITIES AS GOLD.AMENITY_ANALYTICS PRIMARY KEY (METRIC_MONTH, AMENITY_CATEGORY, AMENITY_NAME, LOCATION)
)
DIMENSIONS (
    PUBLIC AMENITIES.AMENITY_CATEGORY AS amenity_category,
    PUBLIC AMENITIES.AMENITY_NAME AS amenity_name,
...
DIMENSIONS (
    ...
    PUBLIC GUESTS.nationality AS nationality,
    PUBLIC GUEST360.LOYALTY_TIER AS loyalty_tier,
...
TABLES (
    PREFS AS SILVER.PREFERENCES_CONSOLIDATED PRIMARY KEY (guest_id),
    GUESTS AS BRONZE.GUEST_PROFILES PRIMARY KEY (guest_id),
    GUEST360 AS GOLD.GUEST_360_VIEW_ENHANCED PRIMARY KEY (GUEST_ID)
)
RELATIONSHIPS (
